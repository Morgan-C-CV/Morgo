...
fn approved_plan_runtime_overlay_updates_with_live_orchestration_state() {
    ...
    let second = plan_manager
        .add_step("Execute task linkage", Some("materialize runtime orchestration"))
        .expect("add second step");

    let task_list_manager = Arc::new(TaskListManager::default());
    task_list_manager.create("Inspect current state", "verify live runtime overlay", None, None, Some(first.id.clone()));
    task_list_manager.create("Execute task linkage", "materialize runtime orchestration", None, None, Some(second.id.clone()));

    let runtime_tasks = Arc::new(TaskManager::default());
    let runtime_task = runtime_tasks.create("execute linked runtime work", "plan-live-session", InteractionSurface::Cli);
    ...
    runtime_tasks.set_validation_state(&runtime_task.id, Some(ValidationState::PendingVerification));
    ...
    assert!(running.contains(&format!("runtime group: {} — group {} still in progress", second.id, second.id)));
    ...
    runtime_tasks.complete(&runtime_task.id, &rust_agent::interaction::dispatcher::NotificationDispatcher::new(
    ...
    assert!(completed.contains(&format!("runtime group: {} — group {} is ready for synthesis", second.id, second.id)));
    assert!(completed.contains("runtime task: task-0 [Completed] role=implement phase=verify validation_state=verified"));
