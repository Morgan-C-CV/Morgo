...
pub fn bundled_skills() -> Vec<SkillDefinition> {
    vec![SkillDefinition {
        ...
        description: "Configure settings.json behavior for Claude Code style automation."
            .to_string(),
        when_to_use: Some(
        ...
        argument_hint: None,
        workflow_hint: Some(
            "Inspect current settings first, then make the smallest targeted change.".to_string(),
        ),
        ...
        content:
            "Use the update-config flow to inspect and modify settings-backed automation behavior."
                .to_string(),
